...
def main():
    ...
    if not os.path.exists(search_script):
        ...
        print(
            "Make sure 'hyperparam_search.py' is in the same folder as this script.\n"
        )
        sys.exit(1)

    parser = argparse.ArgumentParser(
        description="Run parallel PPO hyperparameter search with Optuna."
    )
    ...
    try:
        ...
        study = optuna.load_study(
            study_name="ppo_parallel_search", storage="sqlite:///catan_hyperparams.db"
        )
        ...
        print(
            "Could not print summary: 'optuna' library not found in this environment."
        )
