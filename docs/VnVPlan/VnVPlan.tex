...
\subsection{Summary}
 ...
 It includes a learning agent trained in a simulated
  ...
  Out of scope are large-scale usability studies and extensive performance
   ...
   project’s time and resource constraints. The Computer Vision module is also
    out of scope, as it was not implemented in the final system; therefore, no
     ...
     correct integration and usage within the RLCatan system.
...
\begin{itemize}
    ...
    \item \textbf{User-Centric Validation (Usability Survey):} A partial usability survey will be conducted (using the final class presentation or an informal user testing session with competitive $\Catan{}$ players) to validate the $\NFR{}$ related to usability (NFR.S.2) and the clarity of the $\AI{}$ advice. Survey questions will be included in the Appendix. The target metric is an average rating of at least 4/5 for clarity, ease of navigation, and understanding of $\AI{}$ feedback.
    \item \textbf{External Data Validation:} The $\RL{}$ agent's performance will be validated longitudinally by running thousands of game simulations and comparing its win rate and strategic output against existing benchmark $\Catan{}$ opponents (e.g., baseline bots), and potentially against human gameplay data provided by competitive players. This output will be summarized in the \textbf{Performance Report} extra. The primary metric for this validation is improved win rate and stable strategic output over the benchmark runs.
\end{itemize}
...
The tests are categorized by the major functional areas of the AlgoCatan system to ensure comprehensive coverage.
...
\item{\hypertarget{T.FR.AI.1.1}{T.FR.AI.1.1}\\}
...
\item{\hypertarget{T.FR.AI.1.2}{T.FR.AI.1.2}\\}
...
\textbf{Output:} The $\AI{}$ must provide the suggestion to \texttt{BuildSettlement} at the winning node, with a confidence score greater than 0.85 ($\hyperref[FR.S.3.4]{FR.S.3.4}$). The textual reasoning should explicitly state that this move secures the victory.
...
\item{\hypertarget{T.FR.AI.1.3}{T.FR.AI.1.3}\\}
\textbf{Control:} Automatic
...
\item{\hypertarget{T.FR.UI.2.1}{T.FR.UI.2.1}\\}
...
\textbf{Output:}
    a) The UI must reflect the change within less than 0.1 seconds, showing the new road on the board visualization ($\hyperref[FR.S.4.1]{FR.S.4.1}$) and Player A's resource inventory accurately reduced ($\hyperref[FR.S.4.2]{FR.S.4.2}$).
...
\item{\hypertarget{T.FR.UI.2.2}{T.FR.UI.2.2}\\}
\textbf{Control:} Automatic
...
\textbf{Output:} The system successfully returns the list of 50 turn records, including the dice roll, and a timestamped record of every development card purchased, demonstrating access to historical data ($\hyperref[FR.S.7.3]{FR.S.7.3}$).
...
\subsubsection{Computer Vision and Game State Capture(DEFERRED)} 
...
\item{\hypertarget{T.FR.CV.3.1}{T.FR.CV.3.1}\\}
\textbf{Control:} Manual
...
\item{\hypertarget{T.FR.CV.3.2}{T.FR.CV.3.2}\\}
\textbf{Control:} Manual
...
\textbf{Output:} The system successfully detects the presence of Player A's 2 settlements, 1 city, and 8 roads, correctly applying logical constraints to infer the state despite the occlusion. The system provides a diagnostic confidence metric ($\hyperref[FR.S.1.5]{FR.S.1.5}$) for the occluded area, and must be $\leq 0.6$.
...
\item{\hypertarget{T.FR.CV.3.3}{T.FR.CV.3.3}\\}
\textbf{Control:} Manual
...
\begin{table}[h!]
  ...
  \hline
  \hyperlink{T.FR.AI.1.1}{T.FR.AI.1.1} & $\hyperref[FR.S.3.1]{FR.S.3.1}$, $\hyperref[FR.S.3.2]{FR.S.3.2}$, $\hyperref[FR.S.2.1]{FR.S.2.1}$, $\hyperref[FR.S.2.5]{FR.S.2.5}$, $\hyperref[FR.Sa.3]{FR.Sa.3}$ \\
  \hline
  \hyperlink{T.FR.AI.1.2}{T.FR.AI.1.2} & $\hyperref[FR.S.3.1]{FR.S.3.1}$, $\hyperref[FR.S.3.4]{FR.S.3.4}$ \\
  \hline
  \hyperlink{T.FR.AI.1.3}{T.FR.AI.1.3} & $\hyperref[FR.S.2.1]{FR.S.2.1}$ \\
  \hline
  \hyperlink{T.FR.UI.2.1}{T.FR.UI.2.1} & $\hyperref[FR.S.4.1]{FR.S.4.1}$, $\hyperref[FR.S.4.2]{FR.S.4.2}$, $\hyperref[FR.S.5.1]{FR.S.5.1}$, $\hyperref[FR.S.5.2]{FR.S.5.2}$, $\hyperref[FR.S.6.1]{FR.S.6.1}$, $\hyperref[FR.S.7.4]{FR.S.7.4}$, $\hyperref[FR.S.7.2]{FR.S.7.2}$ \\
  \hline
  \hyperlink{T.FR.UI.2.2}{T.FR.UI.2.2} & $\hyperref[FR.S.5.5]{FR.S.5.5}$, $\hyperref[FR.S.7.3]{FR.S.7.3}$ \\
  \hline
  \hyperlink{T.FR.CV.3.1}{T.FR.CV.3.1} & $\hyperref[FR.S.1.2]{FR.S.1.2}$, $\hyperref[FR.S.7.1]{FR.S.7.1}$, $\hyperref[FR.S.7.2]{FR.S.7.2}$ \\
  \hline
  \end{tabularx}
\end{table}
...
\textbf{Initial State:} RLCatan server running full stack with no concurrent load.
...
\textbf{Initial State:} RLCatan server running full stack.
...
\textbf{Output/Result:} System restarts and restores previous game state within 60~seconds. If recovery fails within 60 seconds, the system must load the last valid checkpoint and log a recovery failure event.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \hyperlink{T.NFR.P.1}{T.NFR.P.1}   & NFR.S.1 -- Scalability \\ \hline
    \hyperlink{T.NFR.U.1}{T.NFR.U.1}   & NFR.S.2 -- Usability \\ \hline
    ...
    \hyperlink{T.NFR.D.1}{T.NFR.D.1}   & NFR.S.5 -- Data Integrity \\ \hline
    \hyperlink{T.NFR.D.2}{T.NFR.D.2}   & NFR.S.5 -- Data Integrity \\ \hline
    \hyperlink{T.NFR.D.3}{T.NFR.D.3}   & NFR.S.5 -- Data Integrity \\ \hline
    \hyperlink{T.NFR.AV.1}{T.NFR.AV.1}  & NFR.S.6 -- Availability \\ \hline
  \end{tabularx}
\end{table}
