...
2026-04-05 & Jake Read & Updated doc with new hazards and requirements for final doc.\\
...
Our project, AlgoCatan, is a reinforcement learning agent capable of playing \emph{Catan}.
Additionally, we provide a web-based system for playing and analyzing games of \emph{Catan} against bots.
In the current scope, users can play against trained bots through a web interface, replay past games,
and request natural-language explanations of bot decisions.
The system integrates game-state management, backend services, persistent storage, reinforcement learning,
and frontend visualization.
Computer vision for reading a physical board is a deferred future extension rather than part of the
...
damage.
For this project, harm includes incorrect or misleading gameplay information,
invalid bot actions, misleading replay or explanation output, disruption of play,
and loss or corruption of game state or model-related data.


\section{Scope and Purpose of Hazard Analysis}\label{sec:scope-and-purpose-of-hazard-analysis}

The losses that can result from these hazards include disrupted gameplay or replay sessions,
incorrect strategic understanding caused by misleading output, inability to continue a match,
loss of historical game data, and loss of trust in the product.

Since the current implementation does not include live physical-board capture,
hazards tied specifically to camera streaming and image-based board recognition are treated as
deferred/future-scope hazards for traceability, rather than as core operational hazards of the
implemented system.
...
\begin{itemize}
    ...
    \item Elo League System
    \item Curriculum Learning Manager
    \item Game State Database
    \item Backend API Server
    \item Explanation Pipeline
    \item Computer Vision Model (Deferred)
\end{itemize}
...
We assume that users of the system are familiar with the rules of \emph{Catan}
and have a basic understanding of how to use a web interface.
We assume that the browser, network connection, and backend services are reliable enough
to support normal gameplay and replay use.
We also assume that the Catanatron-based simulator remains sufficiently accurate to the official game rules,
and that stored model versions and metadata remain loadable and internally consistent.
Finally, we assume that external services such as the LLM provider may fail occasionally,
and that such failures must be handled gracefully by the system.
...
% Finally, a table that spans multiple pages and doesn't look horrendous, only took 2 hours to get the formatting right...
    ...
    \begin{tabularx}{\linewidth}{|P{2.3cm}|Y|Y|Y|Y|P{1.4cm}|P{0.8cm}|}
        ...
        Game State Manager &
        Loss of synchronization between the authoritative simulator state, stored state, and displayed state. &
        The system shows the wrong board state, replay step, or explanation context, disrupting play and analysis. &
        Failed state update, stale frontend state, partial database write, or mismatch between runtime and stored state. &
        Provide a resynchronization mechanism, such as manual correction, reloading the authoritative state, or re-scan in future CV versions. &
        FR.Sa.2 &
        H2-a \\
        ...
        AI-suggested moves are validated against the official rules of \emph{Catan} before display. &
        FR.Sa.3 &
        H3-a \\
        ...
        User Interface &
        Move updates, replay state, or explanation is not displayed on time. &
        Gameplay flow is disrupted and the user may believe the system has failed. &
        Model inference delay, backend/API latency, database lag, or LLM timeout. &
        Limit processing time where possible, timeout early, and provide a clear error message if output cannot be generated. &
        NFR.Sa.1 \newline FR.Sa.4 &
        H4-a \\

        User Interface (Continued) &
        Incorrect or confusing rendering of the game state. &
        Players misinterpret bot actions or system status, leading to confusion. &
        Most recent move is not easy to quickly spot on the displayed board. &
        Visually distinguish the move(s) made most recently, e.g., using highlights or arrows. &
        FR.Sa.7 &
        H4-b \\
        ...
        Game replay option will not load past games correctly. &
        ...
        Backend API Server &
        Dropped connection between frontend and backend services. &
        Gameplay, replay, or explanation requests are interrupted or never delivered. &
        Server outage, network instability, or transport-layer failure. &
        Attempt automatic reconnection and notify the user that the connection has dropped. &
        NFR.Sa.2 \newline FR.Sa.6 &
        H6-a \\
        ...
        LLM fails to generate an explanation. &
        User receives no explanation or an error message, which may lead to frustration. &
        LLM service outage, timeout, or input formatting issue. &
        Provide a clear error message if an explanation cannot be generated, and consider fallback options such as a simpler rule-based explanation. &
        FR.Sa.13 &
        H9-a \\
        ...
        Curriculum progression stalls or regresses. &
        AI fails to improve or even worsens over time, leading to poor performance. &
        - Curriculum criteria are too strict or not well-aligned with learning objectives. \newline - Unintended consequences of curriculum adjustments. &
        Monitor training metrics closely and allow for manual intervention to adjust curriculum parameters if progress stalls. &
        FR.Sa.14 &
        H10-a \\
        ...
        Elo League System &
        Elo ratings become inaccurate or unstable. &
        Self-play matchups become unbalanced, leading to poor training data and suboptimal model performance. &
        - Rating inflation or deflation due to small sample sizes. \newline - Unintended consequences of rating adjustments. &
        Implement safeguards such as minimum match requirements for rating changes, and monitor rating distributions to detect and address instability. &
        FR.Sa.15 &
        H11-a \\
    \end{tabularx}
\end{landscape}
...
\begin{itemize}
    ...
    \item FR.Sa.3: If the AI makes illegal moves, the entire purpose of the system is defeated.
          This requirement is essential to ensure the system provides valid actions.
    ...
    \item FR.Sa.13: LLM failures will likely be a common occurrence during development, so having clear error messages for them will be important for debugging and user experience.
    \item FR.Sa.14: Curriculum-based progress stalling is likely to occur during development, so having a notification system for it will help us identify and address the issue more quickly.
    \item FR.Sa.15: Elo rating instability is likely to occur during development, especially as we make adjustments to the training process, so implementing safeguards for it will help maintain a more stable training environment.
\end{itemize}
...
\begin{itemize}
    \item FR.Sa.1: The computer vision component is a deferred future extension, so the manual confirmation of the parsed board state will also be deferred until that component is implemented.
           This requirement is important for user trust and to prevent incorrect advice, but since the CV component is not in scope for capstone, this requirement will be deferred as well.
    ...
    \item FR.Sa.7: Clear visualization of recent moves is a nice user experience feature, but isn't immediately critical. If it comes up in usability testing we'll handle it.
