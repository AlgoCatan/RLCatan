...
\newcommand{\EloExt}{\href{https://en.wikipedia.org/wiki/Elo_rating_system}{Elo}}
\newcommand{\CurriculumLearningExt}{\href{https://en.wikipedia.org/wiki/Curriculum_learning}{Curriculum Learning}}
\newcommand{\ExperientialLearningExt}{\href{https://en.wikipedia.org/wiki/Experiential_learning}{Experiential Learning}}
...
\section*{Changes from Original SRS Meyer Template}
\addcontentsline{toc}{section}{Changes from Orgiinal SRS Meyer Template}
\begin{table}[H]
...
\begin{itemize}
    ...
    \item \hypertarget{glossary-elo}{\EloExt{}} – A numerical rating system for competitive players (human or AI) that reflects their skill level and enables skill-matched matchmaking. Named after Arpad Elo, the system updates ratings based on game results against opponents of different ratings.
    \item \hypertarget{glossary-curriculum}{\CurriculumLearningExt} – A machine learning training strategy where the model learns progressively through ordered phases of increasing complexity. In this project, curriculum phases simplify game rules initially (e.g., lower victory points) and gradually increase to full complexity.
    \item \hypertarget{glossary-experiential}{\ExperientialLearningExt} – An educational philosophy and learning process where knowledge is created through the transformation of experience, following Kolb's cycle: concrete experience, reflective observation, abstract conceptualization, and active experimentation. In software engineering, this means learning by actually building and deploying systems rather than only studying theory.[15]
\end{itemize}
...
The project aims to create an \AI{} agent, \textbf{RLCatan}, that learns to play \emph{\Catan{}} competitively
using reinforcement learning, and an arena to play against it and existing bots.
The system supports players of all skill levels by providing bot opponents of various difficulties,
via a web interface for gameplay and analysis.
It also serves as a reference implementation for building end-to-end ML systems and an \textbf{experiential learning}
exemplar[15] through hands-on development.
The project addresses large state space complexity, stochasticity, and partial observability via curriculum learning
and self-play.
The digital twin is implemented as a web UI (vision-based sensing remains future work).

\subsection{Current Situation}\label{subsec:current-situation}
Despite its popularity, the inherent complexity of \emph{\Catan{}} presents a significant technical challenge
for \AI{} development.
This complexity limits players' ability to practice against opponents appropriate to their skill level.
Existing \emph{\Catan{}} AI implementations typically offer only a single difficulty level, preventing novice
players from learning fundamentals and denying competitive players access to truly challenging opponents.
Additionally, the technical challenges of training effective \AI{} agents for large state-space games
like \emph{\Catan{}} remain under-documented in software engineering contexts, limiting educational value
for future researchers and companies exploring similar \AI{} systems.
The AlgoCatan project will address these limitations by creating a comprehensive system designed to tackle
these specific technical and usability hurdles.

\subsection{Expected Benefits}\label{subsec:expected-benefits}
\begin{itemize}
    \item {Skill-Appropriate Gameplay} – Varying bot opponents let novices learn fundamentals and advanced players face stronger competition.
    \item {Strategic Insight} – Replay and post-game analysis highlight key decisions and alternatives, and \LLM{} explanations provide natural language insights into strategic choices.
    \item {\AI{} Research Contribution} – Curriculum learning and self-play in a large state-space game provide reusable ML insights.
    \item {Software Engineering Reference} – Demonstrates integration of UI, backend, simulator, and trained models in a production-style architecture.
    \item {Experiential Learning Exemplar} – The team learns by building, testing, and iterating a full ML system; details are elaborated in Section~\ref{subsec:se-ml-learning}.
\end{itemize}

\subsection{Functionality Overview}\label{subsec:functionality-overview}
\begin{itemize}
    \item {\AI{} Agent Training} – A reinforcement learning agent trained through curriculum learning phases and self-play against opponents from an Elo league.
    ...
    \item {\LLM{} Based Strategic Explanations} – Natural language explanations of strategic decisions requestable by the user, generated by a large language model.
    \item {Dynamic Opponent Selection} – Players can select opponents matched to their current skill level, enabling skill-appropriate gameplay.
    ...
    \item {\DigitalTwin{} Representation} – A digital display of the current board and game state, updated in real-time as gameplay progresses (web-based, not vision-based).
\end{itemize}

\subsection{High-Level Usage Scenarios}\label{subsec:high-level-usage-scenarios}
\begin{itemize}
    \item {Scenario 1: Playing Against Skill-Matched \AI{}} – Users select an opponent from the list of bots. Users then play a match through the web interface with real-time board visualization.
    \item {Scenario 2: Multi-Level Player Progression} – A novice player starts against a weaker bot, practices, and gradually challenges higher-rated opponents as their skill improves.
    ...
    \item {Scenario 4: LLM-Based Strategic Explanations} – During or after a game, users can request natural language explanations of strategic decisions made by the \AI{} agent, generated by an integrated large language model.
    \item {Scenario 5: Bot Selection and Benchmarking} – Developers or advanced users select specific trained models from the league, configure them against each other or baselines, and evaluate performance through statistical analysis and match results.
    \item {Scenario 6: Curriculum-Based Training} – Developers run training loops configured with curriculum phases, monitoring phase transitions and reward metrics as the \AI{} agent progresses from simplified to full-complexity game rules.
    \item {Scenario 7: Model Versioning and Rollback} – Developers view historical model versions with their associated Elo ratings, select previous versions for deployment if needed, or analyze performance trends across training iterations.
    \end{itemize}
\begin{figure}[H]
...
\begin{itemize}
    ...
    \item New players seeking to learn the game fundamentals and strategies.
    \item Other Catan bot developers interested in benchmarking their bots against existing models.
...
\begin{itemize}
    ...
    \item User feedback – Iterative testing and feedback from users during development inform usability and functionality requirements.
\end{itemize}


\subsection{User Profiles}\label{subsec:User Profiles}
To ensure the system addresses the needs of its target audience, we have identified three distinct user profiles
within the "Players of \emph{\Catan{}}" stakeholder group, aligned with the Elo system as a skill estimate.
...
\begin{itemize}
    ...
    \item \textbf{\LLM{} Explanations:} Struggles with complex strategic terminology, preferring simpler, actionable explanations. The \LLM{} should provide clear, beginner-friendly insights into why certain moves are recommended.
    \item \textbf{Rationale:} The system's in-game advice functionality and skill-based opponent selection are primarily tailored for this user. The curriculum learning phases trained on simplified rules also benefit novice players who face fewer complex game states early on.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\LLM{} Explanations:} Appreciates more detailed strategic insights, including trade-offs and long-term implications of moves. The \LLM{} should provide intermediate-level explanations that go beyond basic tactics.
    \item \item \textbf{Rationale:} This user benefits most from the system's post-game analysis. The ability to replay games and see win probability changes at each state helps them understand where they went wrong and how to improve.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\LLM{} Explanations:} Seeks in-depth, technical explanations that include probabilistic reasoning, opponent modeling, and meta-strategic considerations. The \LLM{} should provide expert-level insights that can inform high-level play.
    \item \textbf{Rationale:} The \AI{} model's ability to serve as a competitive opponent through the Elo league is crucial for this user profile. They can use the system to play thousands of simulated games against top-rated models to refine their strategies.
\end{itemize}

\subsection{Software Engineering and ML Learning Value}\label{subsec:se-ml-learning}
AlgoCatan is a reference implementation for end-to-end ML system design and an \textbf{experiential learning} exemplar[15].
The team completes Kolb’s cycle (experience, reflection, conceptualization, experimentation) by providing move
explanations and replay features, promoting active learning from experience.

\hspace{1cm}

\noindent\textbf{Key learning outcomes:}
\begin{itemize}
    \item \textbf{Integration Architecture:} Grey-box integration with Catanatron, interface adaptation, and dependency management.
    \item \textbf{Full-Stack ML System Design:} Frontend, backend, training pipeline, model serving, and database working as one system.
    \item \textbf{Advanced Training Methods:} Curriculum learning, Elo-based self-play, and model versioning in practice.
    \item \textbf{Production ML Concerns:} Monitoring, rollback, and reproducible training runs.
\end{itemize}
...
\subsection{Environment Components}\label{subsec:components2}
...
\begin{itemize}
    ...
    \item {Web Server} – Backend that processes game logic, manages Elo ratings, and serves API endpoints to the frontend.
    \item {Database Server} – Database instance storing game history, player profiles, match statistics, Elo ratings, and curriculum progress.
    \item {Training Infrastructure} – Campus GPUs or local machines for running \RL{} training loops with curriculum phases and league-based opponent selection.
    \item {\LLM{} service/API} – an LLM integration that generates natural language explanations of bot actions.
...
\begin{itemize}
    ...
    \item {Device Compatibility} – The web interface must function across major devices and operating systems.
\end{itemize}
...
\begin{itemize}
    \item {Player opponent} – The \AI{} provides an opponent for players to practice against, improving their learning and enjoyment.
    \item {Learning and adaptation} – The \RL{} agent improves over time, indirectly affecting the level of challenge for players.
    \item {Replay and explanations} – The system's analysis and explanation features affect player understanding and strategic insight.
    \item {Device usage} – The system uses computational resources on player devices or servers for inference and visualization.
    \item {Game pacing} – Fast bot moves mean games will typically be shorter than human-only matches, allowing more practice in less time.
\end{itemize}

\subsection{Invariants}\label{subsec:invariants}
Values present in the following invariants are derived from the official rules of \emph{\Catan{}}:
\begin{itemize}
    ...
    \item The total number of resource cards of each type remains exactly 19.
...
\begin{table}[h!]
    ...
    \hline
    NFR.E.1 & The system shall be installable and fully operational on Windows 10+. \label{NFR.E.1} \\
    \hline
    NFR.E.2 & The system shall have a complete installation time of no more than 15 minutes. \label{NFR.E.2} \\
    \hline
    NFR.E.3 & The system shall compile successfully on available GPU/CPU resources. \label{NFR.E.3} \\
    \hline
    NFR.E.4 & The system shall provide \AI{} moves within 1 second. \label{NFR.E.4} \\
    \hline
    NFR.E.5 & The system's Python code shall pass all PEP8 [9] style checks. \label{NFR.E.5} \\
    \hline
    NFR.E.6 & The system's JavaScript/React code shall pass all Google Style Guide [8] checks. \label{NFR.E.6} \\
    \hline
    NFR.E.7 & The system shall enforce the invariant that the total number of roads per player remains exactly 15. \label{NFR.E.7} \\
    \hline
    NFR.E.8 & The system shall enforce the invariant that the total number of settlements per player remains exactly 5. \label{NFR.E.8} \\
    \hline
    NFR.E.9 & The system shall enforce the invariant that the total number of cities per player remains exactly 4. \label{NFR.E.9} \\
    \hline
    NFR.E.10 & The system shall enforce the invariant that the total number of resource cards per type remains exactly 19. \label{NFR.E.10} \\
    \hline
    NFR.E.11 & The system shall enforce the invariant that the total number of development cards remains exactly 25. \label{NFR.E.11} \\
    \hline
    NFR.E.12 & The system shall enforce consistent player turn order throughout the game. \label{NFR.E.12} \\
    \hline
    NFR.E.13 & The system shall not reproduce or distribute copyrighted materials. \label{NFR.E.13} \\
    \hline
    NFR.E.14 & The system shall not use any trademarked assets of \emph{Catan{}}. \label{NFR.E.14} \\
    \hline
    NFR.E.15 & The system shall not collect personally identifiable information. \label{NFR.E.15} \\
    \hline
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Component_Diagram_final.png}
...
\begin{itemize}
    \item Computer Vision Model (Out of Scope): A future component that would process camera input to track physical board state, using techniques like object detection and image segmentation.
    ...
    \item \AI{} Model: The trained model capable of making strategic decisions in \emph{\Catan{}} games through learned policy and value functions.
    ...
    \item User Interface: Web interface providing interactive board visualization, game controls, replay functionality, and strategic analysis displays.

    \item Game State Database: Database for persisting game states, match results, and historical data for replay and analysis.

    \item Backend API Server: An API that handles communication between the UI, game state manager, and game state database.
    ...
    \item Explanation Pipeline: Integration with a large language model (\LLM{}) to generate natural language explanations of strategic decisions made by the \AI{} agent, accessible through the user interface.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.1.1}\textbf{FR.S.1.1 (DEFERRED)} The system shall process live or captured camera input to detect board elements, player actions, and resource placements with at least 99.99\% accuracy.
  \item \label{FR.S.1.2}\textbf{FR.S.1.2 (DEFERRED)} The system shall translate visual features into structured \GameState{} data for the Game State Manager.
  \item \label{FR.S.1.3}\textbf{FR.S.1.3 (DEFERRED)} The system shall detect and report inconsistencies or occlusions in visual view.
  \item \label{FR.S.1.4}\textbf{FR.S.1.4 (DEFERRED)} The system shall dynamically calibrate lighting, camera angle, and board orientation.
  \item \label{FR.S.1.5}\textbf{FR.S.1.5 (DEFERRED)} The system shall provide visual diagnostics and confidence metrics for detected states.
\end{itemize}

\subsubsection{Training Pipeline}

\begin{itemize}
  \item \label{FR.S.2.1}\textbf{FR.S.2.1} The system shall enforce that training bots follow the game rules, move logic, and state transitions consistent with the official \emph{\Catan{}} rule-set.
  \item \label{FR.S.2.2}\textbf{FR.S.2.2} The system shall enable the \AI{} model to perform self-play against opponents selected from the Elo league for training purposes.
  \item \label{FR.S.2.3}\textbf{FR.S.2.3} The system shall provide feedback in the form of rewards, penalties, or success metrics for learning.
  \item \label{FR.S.2.4}\textbf{FR.S.2.4} The system shall support batch and real-time evaluation modes for model benchmarking.
  \item \label{FR.S.2.5}\textbf{FR.S.2.5} The system shall adapt dynamically to changing game conditions through curriculum-guided reinforcement learning.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.3.1}\textbf{FR.S.3.1} The system shall analyze the digital \GameState{} and return a valid action consistent with the rules of \emph{\Catan{}}.
  \item \label{FR.S.3.2}\textbf{FR.S.3.2} The system shall evaluate potential moves based on resource availability, player position, and opponent behavior.
  \item \label{FR.S.3.3}\textbf{FR.S.3.3} The system's models shall interface seamlessly with the Game State, allowing the interchanging of model versions.
  \item \label{FR.S.3.4}\textbf{FR.S.3.4} The system shall win matches against a bot making random moves \textgreater50\% of the time to demonstrate learning.
  \item \label{FR.S.3.5}\textbf{FR.S.3.5} The system shall integrate with the Training Pipeline, enabling continuous improvement.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.3.6}\textbf{FR.S.3.6} The system shall maintain \Elo{} ratings for all trained model snapshots and baseline opponents.
  \item \label{FR.S.3.7}\textbf{FR.S.3.7} The system shall update \Elo{} ratings based on match outcomes between league members using the standard \Elo{} formula.
  \item \label{FR.S.3.8}\textbf{FR.S.3.8} The system shall expose stored league members to the UI, enabling users to select from multiple bot options.
  \item \label{FR.S.3.9}\textbf{FR.S.3.9} The system shall reduce training bias by enforcing fixed probabilities of selecting opponents from known stable benchmarks during training.
  \item \label{FR.S.3.10}\textbf{FR.S.3.10} The system shall manage league member storage and implement pruning strategies to maintain league size to no more than 125 model snapshots.
\end{itemize}

\subsubsection{Curriculum Learning Manager}

\textbf{Functional Requirements}

\begin{itemize}
  \item \label{FR.S.3.12}\textbf{FR.S.3.12} The system shall define and manage curriculum phases with configurable rule variations (e.g., victory point targets, disabled action types).
  \item \label{FR.S.3.13}\textbf{FR.S.3.13} The system shall automatically advance to the next curriculum phase based on metric thresholds and minimum episode requirements.
  \item \label{FR.S.3.14}\textbf{FR.S.3.14} The system shall support optional cycling back to earlier phases or continuous training through the same phase.
  \item \label{FR.S.3.15}\textbf{FR.S.3.15} The system shall log curriculum phase transitions and training metrics for analysis and debugging.
  \item \label{FR.S.3.16}\textbf{FR.S.3.16} The system shall integrate curriculum phases with Elo league opponent selection during training.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.4.1}\textbf{FR.S.4.1} The system shall provide an interactive, real-time visualization of the \emph{\Catan{}} board and current \GameState{}.
  \item \label{FR.S.4.2}\textbf{FR.S.4.2} The system shall display player moves, dice rolls, and resource changes immediately after occurrence.
  \item \label{FR.S.4.3}\textbf{FR.S.4.3} The system shall enable player interactions such as building, trading, and any other actions possible in the game of \emph{\Catan{}}.
  \item \label{FR.S.4.4}\textbf{FR.S.4.4} The system shall present \LLM{}-generated move explanations clearly, with intuitive action selection.
  \item \label{FR.S.4.5}\textbf{FR.S.4.5} The system shall support desktop and mobile layouts for accessibility and responsiveness.
  \item \label{FR.S.4.6}\textbf{FR.S.4.6} The system shall log user interactions and decisions for post-game replay.
  \item \label{FR.S.4.7}\textbf{FR.S.4.7} The system shall be able to load a list of bots from the Elo league.
  \item \label{FR.S.4.8}\textbf{FR.S.4.8} The system shall allow users to select specific opponents from the league.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.5.1}\textbf{FR.S.5.1} The system shall store complete and consistent records of all ongoing and past \emph{\Catan{}} games.
  \item \label{FR.S.5.2}\textbf{FR.S.5.2} The system shall maintain scores, resource inventories, and game metadata.
  \item \label{FR.S.5.3}\textbf{FR.S.5.3} The system shall support efficient read and write operations for real-time synchronization.
  \item \label{FR.S.5.4}\textbf{FR.S.5.4} The system shall ensure data persistence across sessions and support recovery after interruptions.
  \item \label{FR.S.5.5}\textbf{FR.S.5.5} The system shall provide structured access to historical game data for training or replay features.
\end{itemize}


\subsubsection{Backend API Server}

\textbf{Functional Requirements}

\begin{itemize}
  \item \label{FR.S.6.1}\textbf{FR.S.6.1} The system shall enable reliable data exchange between the User Interface, Game State Manager, and Database.
  \item \label{FR.S.6.2}\textbf{FR.S.6.2} The system shall support both synchronous and asynchronous message-passing mechanisms.
  \item \label{FR.S.6.3}\textbf{FR.S.6.3} The system shall ensure low-latency updates (\textless100ms for UI updates, \textless1s for \AI{} decisions) to maintain real-time synchronization of states.
\end{itemize}
...
\begin{itemize}
  \item \label{FR.S.7.1}\textbf{FR.S.7.1} The system shall ensure game state updates consistently follow the rules of \emph{\Catan{}}.
  \item \label{FR.S.7.2}\textbf{FR.S.7.2} The system shall track player assets such as settlements, cities, roads, and resources accurately.
  \item \label{FR.S.7.3}\textbf{FR.S.7.3} The system shall record turn and dice data to enable game history tracking and replay functionality.
  \item \label{FR.S.7.4}\textbf{FR.S.7.4} The system shall automatically update the board state to reflect player actions and maintain consistency.
  \item \label{FR.S.7.5}\textbf{FR.S.7.5} The system shall provide a method for structured access to current board and player information.
\end{itemize}

\subsubsection{Explanation Pipeline}

\textbf{Functional Requirements}

\begin{itemize}
  \item \label{FR.S.8.1}\textbf{FR.S.8.1} The system shall integrate with a large language model (\LLM{}) to generate natural language explanations of the \AI{} agent's strategic decisions.
  \item \label{FR.S.8.2}\textbf{FR.S.8.2} The system shall provide an API endpoint for the frontend to request explanations for specific moves or game states.
  \item \label{FR.S.8.3}\textbf{FR.S.8.3} The system shall ensure explanations are contextually relevant, accurate, and understandable to players of varying skill levels.
\end{itemize}

\subsubsection{Non-Functional Requirements}

\begin{itemize}
  \item \label{NFR.S.1}\textbf{NFR.S.1} \textbf{Scalability:} The system shall support multiple concurrent games without performance degradation.
  \item \label{NFR.S.2}\textbf{NFR.S.2} \textbf{Usability:} The system shall provide an intuitive interface accessible to users (assessed through usability surveys/interviews).
  \item \label{NFR.S.3}\textbf{NFR.S.3} \textbf{Maintainability:} The system's codebase shall follow modular design for easy updates and debugging.
  \item \label{NFR.S.4}\textbf{NFR.S.4} \textbf{Installability:} The system shall ensure compatibility across major operating systems and browsers.
  \item \label{NFR.S.5}\textbf{NFR.S.5} \textbf{Data Integrity:} The system shall prevent data corruption through validation and transactional consistency.
  \item \label{NFR.S.6}\textbf{NFR.S.6} \textbf{Availability:} The system shall have an uptime of at least 99\%.
\end{itemize}
...
\begin{itemize}
    \item A REST API between the React frontend and Flask backend for gameplay, replay, and explanation requests.
    \item A Python library interface to the Catanatron simulator for rule enforcement and game-state transitions.
    \item An \RL{} environment interface compatible with Stable Baselines3 / OpenAI Gym-style interaction for training the \RL{} agent.
    \item An external \LLM{} API interface for generating natural-language move explanations.
\end{itemize}
...
\begin{itemize}
    \item Human Player: Wants a challenging, fair, and responsive game experience.
    \item Developer: Wants to validate that the integrated system behaves correctly during real gameplay.
    \item Society: Benefits from an accessible and engaging board-game \AI{} system.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item User interface is available and connected to the backend API.
    \item A valid \emph{Catan} game configuration can be initialized.
    \item The selected \RL{} agent model is loaded and operational.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item A complete game is played to termination or normal user exit.
    \item All moves, state transitions, and results are recorded for later analysis or replay.
    \item The user is shown the final outcome of the match.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If the game cannot continue, the system preserves logs and any recoverable state.
    \item The user receives a clear error message and the session ends gracefully or falls back to a baseline opponent if available.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item Human player starts a new game session from the web interface.
    \item Backend initializes the game state using the simulation environment and loads the selected opponent.
    \item The current board state is displayed to the user.
    \item Human player submits legal actions through the interface.
    \item After each player action, the game state manager updates the state and synchronizes it with the backend and UI.
    \item On the \RL{} agent's turn, the backend queries the loaded model for a legal action.
    \item The chosen action is applied through the simulator and the updated state is shown to the player.
    \item Steps 4--7 repeat until the game ends.
    \item Final results are displayed and persisted for replay and analysis.
\end{itemize}
...
\noindent\textbf{Use Case Name:} Multi-Level Player Progression

\noindent\textbf{Primary Actor:} Returning Human Player

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Novice and Intermediate Players: Want a difficulty curve that supports learning and improvement.
    \item Competitive Players: Want progressively stronger opponents as their skill increases.
    \item Developer: Wants to verify that difficulty progression is meaningful and usable.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item The bot selection panel contains multiple opponents of varying strength.
    \item The system allows the user to choose any of the available bots.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item The player selects an appropriate next opponent.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item Player opens the opponent selection screen.
    \item System displays available opponents and their relative strengths.
    \item Player selects a bot that matches their desired challenge level.
    \item A new match is launched against the selected bot.
    \item After the match, results are recorded.
\end{itemize}
...
\noindent\textbf{Use Case Name:} Replay of Past Games with Strategic Analysis

\noindent\textbf{Primary Actor:} Human Player or Analyst

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Human Player: Wants to review decisions and learn from previous games.
    \item Developer: Wants access to historical data for debugging and evaluation.
    \item Researcher: Wants structured game histories for studying strategy and model behavior.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item At least one completed game has been stored in the database.
    \item Replay and analysis data are available for retrieval.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item The selected game is replayed accurately from stored history.
    \item The user can inspect important game states and associated strategic analysis.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If replay data is incomplete or unavailable, the system informs the user and preserves all accessible data.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item User opens the replay/history view.
    \item Backend retrieves the stored game states, actions, and metadata.
    \item UI reconstructs the board and allows the user to step forward or backward through turns, or jump to a specific turn number.
    \item At each selected state, the analysis module provides supporting information such as estimated win probability or other evaluation metrics.
    \item User reviews the replay until the desired point or the end of the game.
\end{itemize}
...
\noindent\textbf{Use Case Name:} LLM-Based Strategic Explanation Request

\noindent\textbf{Primary Actor:} Human Player

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Human Player: Wants understandable explanations of why a move was made.
    \item Developer: Wants to verify that explanation requests are correctly routed and logged.
    \item Researcher: Wants to study whether explanations improve interpretability and learning value.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item A game is currently in progress or a past game state is available.
    \item The explanation pipeline and external \LLM{} interface are operational.
    \item Sufficient state and move context are available for the selected explanation target.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item A natural-language explanation is returned to the user and associated with the selected move or state.
    \item The explanation request and result are logged for future review.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If explanation generation fails, the user receives a clear error message and gameplay or replay can continue without interruption.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item User selects a move or game state and requests an explanation.
    \item Backend gathers the relevant game-state context, move details, and any analysis metadata.
    \item Explanation pipeline formats the request for the \LLM{} service.
    \item External \LLM{} API generates a natural-language explanation.
    \item Backend validates and returns the explanation to the frontend.
    \item UI displays the explanation to the user.
\end{itemize}
...
\noindent\textbf{Use Case Name:} Bot Selection and Benchmarking

\noindent\textbf{Primary Actor:} Developer or Advanced User

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Developer: Wants to compare trained models against baselines and other snapshots.
    \item Researcher: Wants reproducible benchmark results across different opponents or configurations.
    \item Competitive Player: Wants to inspect the relative strength of available bots.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item Multiple baseline and trained models are available in the league or model store.
    \item Benchmarking configuration options are accessible to the user.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item Selected models are evaluated in the requested configuration.
    \item Match results, statistics, and rating impacts are recorded.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If a selected model cannot be loaded or executed, the benchmark is aborted with logs and an error message.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item Developer opens the benchmarking interface (Interface deferred for scope of project. Can still run from CLI directly).
    \item Developer selects one or more opponents and benchmark settings such as number of games or evaluation mode.
    \item Backend loads the chosen models and initializes benchmark matches in the CLI interface.
    \item Matches are executed and results are aggregated.
    \item System displays statistics such as wins, losses, and updated ratings where applicable.
\end{itemize}
...
\noindent\textbf{Use Case Name:} Curriculum-Based Training Run

\noindent\textbf{Primary Actor:} Developer

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Developer: Wants a controlled training workflow with measurable phase progression.
    \item Researcher: Wants to study the impact of curriculum design on learning quality.
    \item Project Team: Wants reproducible training runs and interpretable training logs.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item A valid curriculum configuration has been defined.
    \item Training infrastructure and required model dependencies are available.
    \item The simulator and league integration are operational.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item Training completes or advances through one or more curriculum phases.
    \item Metrics, checkpoints, and phase transitions are recorded.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If training fails, partial logs and the most recent valid checkpoint are preserved when possible.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item Developer launches a training run with a selected curriculum configuration.
    \item Curriculum manager initializes the first phase and its rule variations.
    \item Training pipeline selects opponents from the league and begins self-play episodes.
    \item Rewards, episode results, and other training metrics are collected.
    \item System checks whether advancement criteria for the current phase have been met.
    \item If criteria are met, the curriculum manager advances to the next phase.
    \item Updated checkpoints and logs are saved throughout the run.
    \item Training stops at completion or at a developer-defined stopping point.
\end{itemize}
...
\noindent\textbf{Use Case Name:} Model Versioning and Rollback

\noindent\textbf{Primary Actor:} Developer or Administrator

\noindent\textbf{Stakeholders and Interests:}
\begin{itemize}
    \item Developer: Wants safe deployment and the ability to recover from regressions.
    \item Project Team: Wants clear traceability between checkpoints, ratings, and performance history.
    \item Advanced User: May want to inspect how different model versions behave.
\end{itemize}

\noindent\textbf{Preconditions:}
\begin{itemize}
    \item Multiple model checkpoints or snapshots have been saved.
    \item Metadata such as version identifiers, ratings, or training history is available.
\end{itemize}

\noindent\textbf{Postconditions (Success Guarantees):}
\begin{itemize}
    \item A selected previous model version becomes active for evaluation or deployment.
\end{itemize}

\noindent\textbf{Postconditions (Failure Guarantees):}
\begin{itemize}
    \item If a selected version is missing or invalid, the current stable version remains active and the user is notified.
\end{itemize}

\noindent\textbf{Main Success Scenario (Basic Flow):}
\begin{itemize}
    \item Developer selects a previous version for inspection or rollback.
    \item System validates that the selected model artifact is available and loadable.
    \item Developer modifies loaded model metadata if necessary (e.g., updating the active version reference).
    \item Backend updates the active model reference.
    \item The chosen version is used for subsequent gameplay, evaluation, or deployment tasks.
\end{itemize}
...
\textbf{Reasoning} & \textbf{Date} \\
\hline
\hyperref[FR.S.3.1]{FR.S.3.1} & Action Return & M &
\parbox[t]{4cm}{Analyzes the \GameState{} and returns valid moves.} &
2025-12-06 \\
\hline
\hyperref[FR.S.3.2]{FR.S.3.2} & State Considerations & S &
\parbox[t]{4cm}{Improves predictions using more complex decision data.} &
2025-12-06 \\
\hline
\hyperref[FR.S.3.3]{FR.S.3.3} & Model Versioning & C &
\parbox[t]{4cm}{Allows models to be switched easily.} &
2025-12-12 \\
\hline
\hyperref[FR.S.3.4]{FR.S.3.4} & Baseline Success & M &
\parbox[t]{4cm}{Ensures model is actually learning.} &
2025-12-18 \\
\hline
\hyperref[FR.S.3.5]{FR.S.3.5} & Continuous Improvement & M &
\parbox[t]{4cm}{Allows the model to continue training after stopping.} &
2026-04-05 \\
\hline
\end{tabular}
...
2026-01-15 \\
\hline
\hyperref[FR.S.3.8]{FR.S.3.8} & Dynamic Opponent Selection & S &
\parbox[t]{4cm}{Enables players to select opponents matched to their skill level.} &
2026-01-22 \\
\hline
\hyperref[FR.S.3.9]{FR.S.3.9} & Fixed Baseline Sampling & S &
\parbox[t]{4cm}{Reduces training bias via opponent selection.} &
2026-01-22 \\
\hline
...
2025-11-06 \\
\hline
\hyperref[FR.S.4.2]{FR.S.4.2} & Visual Indicators & S &
\parbox[t]{4cm}{Shows player resources, turns, and actions for clarity.} &
2025-11-06 \\
\hline
\hyperref[FR.S.4.3]{FR.S.4.3} & Action Controls & M &
\parbox[t]{4cm}{Allows players to build, trade, and manage turns easily.} &
2025-11-06 \\
\hline
\hyperref[FR.S.4.4]{FR.S.4.4} & \AI{} Suggestion Display & C &
\parbox[t]{4cm}{Provides a clear interface for \LLM{} explanations.} &
2025-11-06 \\
\hline
\hyperref[FR.S.4.5]{FR.S.4.5} & Multi-Platform Support & S &
...
2025-10-22 \\
\hline
\hyperref[FR.S.5.3]{FR.S.5.3} & Synchronization & M &
\parbox[t]{4cm}{Synchronizes database updates efficiently.
actions.} &
2025-10-30 \\
\hline
\hyperref[FR.S.5.4]{FR.S.5.4} & Data Integrity & M &
\parbox[t]{4cm}{Prevents corruption and ensures consistency between
tables.} &
2025-10-24 \\
\hline
\hyperref[FR.S.5.5]{FR.S.5.5} & Historical Logging & C &
...
2025-11-09 \\
\hline
\hyperref[FR.S.6.2]{FR.S.6.2} & Message Passing & S &
\parbox[t]{4cm}{Provides support for async and synchronous message-passing.} &
2025-11-13 \\
\hline
\hyperref[FR.S.6.3]{FR.S.6.3} & Low Latency & S &
\parbox[t]{4cm}{Ensures updates don't take too long.} &
2025-11-14 \\
\hline
\end{tabular}
...
\textbf{Reasoning} & \textbf{Date} \\
\hline
\hyperref[FR.S.7.1]{FR.S.7.1} & Following Rules & M &
\parbox[t]{4cm}{Ensures that only legal \emph{\Catan{}} moves are processed.} &
2025-10-27 \\
\hline
...
2025-10-27 \\
\hline
\hyperref[FR.S.7.5]{FR.S.7.5} & Query Interface & M &
...
\textbf{Reasoning} & \textbf{Date} \\
\hline
\hyperref[FR.S.8.1]{FR.S.8.1} & \LLM{} Integration & M &
\parbox[t]{4cm}{Required for explanations to be generated.} &
2026-04-05 \\
\hline
\hyperref[FR.S.8.2]{FR.S.8.2} & API Endpoint & M &
\parbox[t]{4cm}{Required to receive and process explanation requests.} &
2026-04-05 \\
\hline
\hyperref[FR.S.8.3]{FR.S.8.3} & Explanation Quality & S &
\parbox[t]{4cm}{Ensures explanations are actually relevant.} &
2026-04-05 \\
\hline
\end{tabular}
...
To validate compatibility, simulation interoperability testing will be performed.
The \RL{} agent will be tested in multiple versions of the environment.
The validation of the functional requirements outlined for the AlgoCatan agent will be conducted throughout
the development lifecycle, with a strong focus on acceptance testing and simulation-based evaluation.

Early validation efforts will involve the use of simplified \emph{\Catan{}} game environments and prototypes
of the \RL{} agent's architecture.
Key stakeholders, such as \AI{} researchers, game designers, and domain experts,
will participate in these early-stage reviews.
...
\begin{itemize}
    ...
    \item {LLM API} - Gemini API for integration of natural language processing capabilities.
    \item {Game Simulator} - The open-source Catanatron library will be used for game state simulation.
\end{itemize}
...
\begin{table}[htbp]
  ...
  EXPO Demonstration & April 7 \\ \hline
...
\end{tabular}
\caption{Tasks and Deliverables (As originally planned)}
...
\begin{itemize}
    \item {Primary Programming Language} – Python for backend and \AI{} development; TypeScript/JavaScript for frontend.
    ...
    \item {Web Framework} – Flask with Flask-CORS for REST API development and server-side game logic.
    \item {Frontend Framework} – React with TypeScript and Vite as the build tool for the web interface.
    \item {Database} – PostgreSQL for persistent game state storage and historical data management.
    \item {Containerization} – Docker and Docker Compose for deployment and environment consistency.
    \item {\CV{} Libraries} – (Deferred) OpenCV and YOLOv9 planned for future computer vision integration.
...
\begin{itemize}
    ...
    \item FR.Sa.3: The system shall validate all AI moves against the official rules of \emph{Catan} before display.
    \item FR.Sa.4: The system shall provide a clear error message if UI updates cannot be generated.
    \item FR.Sa.5: The system shall ensure current game state is saved correctly before requesting state updates from the simulator.
    \item FR.Sa.6: The system shall attempt automatic reconnection at least 3 times before failing.
    \item FR.Sa.7: The system shall visually distinguish the most recent move(s).
    ...
    \item FR.Sa.11: The system shall restrict image capture and processing to the game board region, and shall ignore, discard, or mask non-board content such as player faces or personal items when detected.
    \item FR.Sa.12: The system shall automatically validate iterations of the reinforcement learning environment to ensure its simulation dynamics remain consistent with real gameplay conditions.
    \item FR.Sa.13: The system shall provide a clear error message or fallback simple explanation if the LLM API cannot be reached.
    \item FR.Sa.14: The system shall notify developers if curriculum-based progress stalling occurs.
    \item FR.Sa.15: The system shall implement safeguards to prevent Elo rating instability, such as minimum match requirements for rating changes. \newline
\end{itemize}
...
\begin{enumerate}
    ...
    several elements not explicitly covered by the Meyer template.
     ...
     align with the rubric’s expectations. We decided to create
      a “Changes from Template” table that clearly outlined all modifications and
    ...
    \item We spoke extensively with our supervisor, Dr. David,
     ...
     reinforcement learning (RL) and deep reinforcement learning (DRL).
      ...
      \emph{\Catan{}} player, which allowed us to get direct user input.
...
\begin{enumerate}
    ...
    Finally, mastering computer vision (CV) will be crucial for processing visual input from a camera to represent the game state.
