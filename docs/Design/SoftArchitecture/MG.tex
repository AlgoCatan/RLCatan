...
the SRS.
To satisfy the requirements for real-time operation (NFR.E.4), low-latency communication (FR.S.6.3), and component decoupling (NFR.S.3), a design decision was made to use a message queue middleware (as implemented in \mref{mQueue}) for asynchronous communication between the primary components.
This allows the CV, AI, and UI modules to operate independently without blocking each other.
...
A module A uses a module B if the correct execution of A depends upon the availability of a correct implementation of B. For our system, the 'uses' relation is not a simple hierarchy, as the main components communicate in a decoupled manner through the Communication Layer (\mref{mQueue}).
The relationships are shown in Figure \ref{FigUH}. This graph is a directed acyclic graph (DAG), as required.
